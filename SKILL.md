---
name: curiousbuddy
description: 阿里云 OSS + 通义千问 API 集成技能，支持微信渠道的 AI 解题/批改和 TRSL 中英翻译功能。
---

# curiousbuddy OpenClaw Skill

阿里云 OSS + 通义千问 API 技能（解题/批改）+ TRSL 中英翻译。
...
4. **TRSL 翻译模式** — 中文文本/语音实时翻译为英文 + 语音合成
...
| `scripts/trsl_handler.py` | TRSL 翻译模式处理器 |
...
| `scripts/test_trsl_handler.py` | TRSL 功能测试 |
...
| 变量 | 说明 |
|------|------|
| `ANTHROPIC_AUTH_TOKEN` | MiniMax API 密钥（推荐） |
| `MINIMAX_API_KEY` | MiniMax API 密钥（备选，向后兼容） |
| `ANTHROPIC_BASE_URL` | API 端点（默认 `https://api.minimaxi.com/anthropic`） |
| `OSS_ACCESS_KEY` | 阿里云 AccessKey ID |
| `OSS_SECRET` | 阿里云 AccessKey Secret |
| `DASHSCOPE_API_KEY` | 通义千问 API 密钥 |

环境变量优先于 `env_config.json`。生产部署建议使用环境变量，而非配置文件。

**`env_config.json` 备份说明**：如果同时存在环境变量和配置文件，代码优先读取环境变量，配置文件仅作向后兼容。

**文件权限**：确保 `~/.openclaw/memory/env_config.json` 权限为 `600`（仅所有者可读写）：
```bash
chmod 600 ~/.openclaw/memory/env_config.json
```
